function X= LFBBFGS_apply_imerse_free(opts,metric, c,U)
...
lB = length(metric.Ds);
alpha= L0_apply(opts,1);
UT = zeros(length(c) );
if(lB >=1)
      HD = metric.HeDs{1};
      % Z = metric.Rs{1}'\((metric.HeDs{1})'*U);
      Z = alpha*metric.Rs{1}'\(HD(c,:)');
      W = (metric.Rs{1})\Z;
      for j = 1:length(opts.metric.cs{1}) 
          Ucs(j,:) = (opts.metric.cs{1}(j) == c);
      end
%      metric.Rs{i} Z =  U(opts.metric.cs{i},:) ;
      Z = (metric.Rs{1})\Ucs; 
      % Still need to eventually form X explicitly!
      X = X+ opts.metric.Ds{1}*(Z-W);
end
for i =2:lB
